...
const uploadImages = async (images) => {
    ...
    // Filter out any failed uploads (i.e., null values)
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (req.body.images && req.body.images.length > 0) {
            imgurl = await uploadImages(req.body.images);
            if (!imgurl || imgurl.length === 0) {
                return res.status(500).json({
                    error: 'Error in uploading images',
                    success: false,
        ...
        const updatedCategory = await Category.findByIdAndUpdate(
            ...
            {
                name: req.body.name,
                images: imgurl.length > 0 ? imgurl : undefined, // Keep existing images if no new ones
                color: req.body.color,
            },
            { new: true, omitUndefined: true } // Skip undefined fields
        );
